...
(function() {
    ...
    function table(el, dest) {
        ...
        q.onload = function() {
            try{
                loadTable(JSON.parse(this.responseText), dest);
                
            } catch(e) {
                dest.innerHTML = "<span class='em-error'>Failed to load. " + e + "</span>";
            }
            if (el.closest('.clps-o')) el.closest('.clps-o').dispatchEvent(new Event('change', {
                bubbles: true
            }));
        };
    }

    function loadTable(arr, el) {
        var out = "",
            i,
            x,
            head = [],
        ...
        if (total > 10) {
            ...
            if (page - 3 > 0) {
                paginate[++l] = '<button class="em-tablePagItem" data-src="' + el.getAttribute('data-src') + '?s=' + sort + '">first</button>';
                paginate[++l] = '<button class="em-tablePagItem" data-src="' + el.getAttribute('data-src') + '?p=' + (page - 1) + '&s=' + sort + '"><span style="transform:rotate(-90deg);display:inline-block">▲</span></button>';
            }

            for (q = 1; q <= pages; q++) {
                if (q <= Math.max(page + 2, 5) && (q >= Math.max(page - 2, 0) || pages - q < 5)) {
                    paginate[++l] = '<button class="em-tablePagItem';
                    if (q == page) paginate[++l] = ' disabled';
                    paginate[++l] = '" data-src="' + el.getAttribute('data-src') + '?p=' + q + '&s=' + sort + '">' + q + '</button>';
                }

                if (q >= Math.max(page + 2, 5) && q < pages) {
                    paginate[++l] = '<button class="em-tablePagItem" data-src="' + el.getAttribute('data-src') + '?p=' + (page + 1) + '&s=' + sort + '"><span style="transform:rotate(90deg);display:inline-block">▲</span></button>';
                    paginate[++l] = '<button class="em-tablePagItem" data-src="' + el.getAttribute('data-src') + '?p=' + pages + '&s=' + sort + '">last</button>';
                    break;
                }
            }
            ...
            for (x = 0; x < head.length; x++) {
                sortSelect[++q] = '<a class="em-tableSortOption desc" data-src="' + el.getAttribute('data-src') + '?p=' + page + '&s=' + head[x] + '.desc"><div class="em-tableSortArrow">▲</div>' + head[x] + '</a>';
                sortSelect[++q] = '<a class="em-tableSortOption asc" data-src="' + el.getAttribute('data-src') + '?p=' + page + '&s=' + head[x] + '.asc"><div class="em-tableSortArrow">▼</div>' + head[x] + '</a>';
            }

            r[++j] = '<div class="em-tableGetRows"><div>';
            r[++j] = paginate.join('');
            r[++j] = '<div class="em-tableCurrentRows">Showing records ' + (pageMessage - 9) + '-' + Math.min(pageMessage, total) + ' of ' + total + '.</div>';
            r[++j] = '<div class="em-tableSort">Sort by<button class="em-tableSortButton"><div class="em-tableSortArrow">';
            ...
            r[++j] = '</button><div class="em-tableSortSelect">' + sortSelect.join('') + '</div></div>';
            r[++j] = '</div></div>';
        } // table
        ...
        for (i = 0; i < head.length; i++) {
            ...
            if (arr.length > 1) {
                r[++j] = '<svg class="icon-sm" viewBox="0 0 320 512" xmlns="http://www.w3.org/2000/svg"><path d="M41 288h238c21.4 0 32.1 25.9 17 41L177 448c-9.4 9.4-24.6 9.4-33.9 0L24 329c-15.1-15.1-4.4-41 17-41zm255-105L177 64c-9.4-9.4-24.6-9.4-33.9 0L24 183c-15.1 15.1-4.4 41 17 41h238c21.4 0 32.1-25.9 17-41z"></path></svg>';
            }
        ...
        if (arr.length < 1 || total == 0) {
            r[++j] = '<tr><td colspan="' + (head.length || 1) + '">' + empty_msg + '</td></tr>';
        } else {
        ...
        el.innerHTML = r.join(''); // add events
        // copy
        ...
        (function(reload, el) {
            reload.addEventListener('click', function() {
                el.innerHTML = '<div class="loader-typing"><img src="/static/img/typing.gif"></div>';
                table(el);
            });
        })(reload, el); // sort
                ...
                (function(h, el) {
                    h.addEventListener('click', function() {
                        var table = el.querySelector('tbody'),
                            r = Array.from(table.querySelectorAll('tr')).sort(comparer(index(h) - 1));
                        this.asc = !this.asc;
    ...
    var comparer = function comparer(index) {
            return function(a, b) {
                ...
                if (/^\d{1,2}\/\d{1,2}\/\d{2,4}$/.test(valA) && /^\d{1,2}\/\d{1,2}\/\d{2,4}$/.test(valB)) return new Date(valA) - new Date(valB);
