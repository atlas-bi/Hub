...
(function() {
    ...
    if (myTextArea) var mySourceCodeMirror = CodeMirror.fromTextArea(myTextArea, {
        theme: "nord",
        mode: "sql",
        autoRefresh: true,
    ...
    if (myProcessingTextArea) var myProcessingCodeMirror = CodeMirror.fromTextArea(myProcessingTextArea, {
        theme: "nord",
        mode: "python",
        autoRefresh: true,
        viewportMargin: Infinity,
    ...
    d.addEventListener('click', function(e) {
        ...
        if (e.target.closest('#project_cron_check')) {
            ...
            if (i.getAttribute("checked") == "checked") {
                ...
                i.setAttribute('checked', 'checked');
                ipt.value = 1;
                document.querySelector('.task-destinationZip').style.removeProperty('display');
                // add required attr on inputs
            }
            ...
            q.onload = function() {
                task = d.querySelectorAll('.em-drop[data-task]')[d.querySelectorAll('.em-drop[data-task]').length - 1].getAttribute('data-task');
                task++;
                document.getElementById('em-elementGroup').insertAdjacentHTML('beforeend', q.responseText.replace(/task1/gm, 'task' + task).replace(/data-task="1"/gm, 'data-task="' + task + '"'));
                // update titles
                drop = document.querySelectorAll('.em-drop[data-task');
                for (var x = 0; x < drop.length; x++) {
                    drop[x].innerHTML = "Stop " + numberToEnglish(x + 1, '') + '<svg class="em-dropIcon" viewBox="0 0 320 512" xmlns="http://www.w3.org/2000/svg"><path d="m143 352.3-136-136c-9.4-9.4-9.4-24.6 0-33.9l22.6-22.6c9.4-9.4 24.6-9.4 33.9 0l96.4 96.4 96.4-96.4c9.4-9.4 24.6-9.4 33.9 0l22.6 22.6c9.4 9.4 9.4 24.6 0 33.9l-136 136c-9.2 9.4-24.4 9.4-33.8 0z"/></svg>';
                }
            };
        }
    });
    ...
    d.addEventListener('change', function(e) {
        ...
        if (e.target.closest('select[name="smb_destination"]')) {
            ...
            if (t.value == 'none') {
                t.parentElement.querySelector('.' + t.getAttribute('data-smb')).innerHTML = '';
                return;
            }
            q = new XMLHttpRequest();
            ...
            if (t.value == 'none') {
                t.parentElement.querySelector('.' + t.getAttribute('data-sftp')).innerHTML = '';
                return;
            }
            q = new XMLHttpRequest();
            ...
            q.send();
            q.onload = function() {
                var p = t.parentElement.querySelector('.' + t.getAttribute('data-sftp'));
                p.innerHTML = q.responseText;
            ...
            if (t.value == 'none') {
                t.parentElement.querySelector('.' + t.getAttribute('data-database')).innerHTML = '';
                return;
            }
            q = new XMLHttpRequest();
            ...
            q.send();
            q.onload = function() {
                var p = t.parentElement.querySelector('.' + t.getAttribute('data-database'));
                p.innerHTML = q.responseText;
            ...
            if (t.value == 1) {
                ...
                p.querySelector('.task-sourceSftpQuery').style.removeProperty('display');
            } else if (t.value == 6) {
        ...
        if (typeof myProcessingCodeMirror != 'undefined') myProcessingCodeMirror.refresh();        
