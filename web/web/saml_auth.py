...
def idp_initiated() -> Response:
    ...
    try:
        ...
        groups = identity.get(app.config["SAML_ATTR_MAP"]["groups"]) or []

        if "REQUIRED_GROUPS" in app.config and app.config["REQUIRED_GROUPS"] and not bool(
            set(app.config["REQUIRED_GROUPS"]) & set(groups)
        ):
