(function (rhcTable, $, undefined) {
  ...
  function loadTable(arr, el, enableReload) {
    ...
    if (total > page_size) {
      ...
      var paginate = [],
        l = -1;
      paginate[++l] = '<nav class="level my-3"><div class="level-left"><div class="level-item"><nav class="pagination  is-close em-tablePag" role="navigation" aria-label="pagination"><ul class="pagination-list">';
    ...
    r[++j] = '<div class="table-container"><table class="table is-narrow is-fullwidth sort ' + theme + '"><thead>';
   ...
   // add head to table
    ...
    r[++j] = "</table></div>";
    r[++j] = "<div class='has-text-weight-light has-text-grey em-tableUpdated'>" + current_date + "</div></div>";
    ...
    if(el.querySelector('.em-tableSort select')){
        el.querySelector('.em-tableSort select').addEventListener("change", function(e){
