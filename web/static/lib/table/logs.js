      ...
      var reloadTimer,
        get_new_logs = function () {
          if (window.ajaxOn == true) {
            ...
            last_id = container
              .querySelector('nav.log-group:last-of-type div[log_id]')
              .getAttribute('log_id');
            q = new XMLHttpRequest();
            q.open('get', url + '?gte=' + last_id, true);
            q.send();
  ...
  function appendLogs(arr, container) {
    ...
    for (var e = 0; e < log_id_els.length; e++) {
      ...
      log_id_min =
        log_id_min == 0
          ? parseInt(log_id)
          : Math.min(parseInt(log_id), log_id_min);
    }
    ...
    for (x = 0; x < arr.length; x++) {
      ...
      var existing_log = container.querySelector(
        'div[log_id="' + v['log_id'] + '"]',
      );
      if (existing_log) {
        ...
        if (status == v['status'] && job_id == v['job_id']) {
          ...
          section['message'] = [
            { message: v['message'], date: v['date'], log_id: v['log_id'] },
          ];
          status = v['status'];
          job_id = v['job_id'];
    ...
    for (var x = 0; x < group.length; x++) {
      ...
      level.classList.add('level', 'mb-1', 'mx-3', 'is-align-items-start');
      ...
      nav.addEventListener('mouseup', function (e) {
        ...
        var angle = this.querySelector(
          '.icon i.fa-angle-right, .icon i.fa-angle-down',
        );
      ...
      for (var m = 0; m < group[x]['message'].length; m++) {
        ...
        date.innerHTML =
          group[x]['job_id'] + ' ' + group[x]['message'][m]['date'];
