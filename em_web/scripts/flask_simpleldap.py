"""
    Modification of script by https://github.com/alexferl/flask-simpleldap to work better 
    with some unicode LDAP values and also save more items into global vars
"""
# flake8: noqa
...
class LDAP(object):
    ...
    def init_app(app):
        ...
        if app.config["LDAP_REQUIRE_CERT"]:
            ...
            ldap.set_option(
                ldap.OPT_X_TLS_CACERTFILE, current_app.config["LDAP_CERT_PATH"]
            )
        ...
        def wrapper(func):
            ...
            def wrapped(*args, **kwargs):
                ...
                match = [
                    group
                    for group in groups
                    if session["ldap_groups"]
                    and group.encode("utf-8") in session["ldap_groups"]
                ]
                if not match:
                    abort(401)
    ...
    def basic_auth_required(self, func):
        ...
        def wrapped(*args, **kwargs):
            ...
            session["ldap_username"] = req_username
            # g.ldap_password = req_password
            session["ldap_password"] = req_password
