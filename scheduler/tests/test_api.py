...
def test_add_task(client_fixture: fixture) -> None:
    ...
    scheduled_task = apscheduler.get_job(f"{p_id}-{t.id}-cron")
...
def test_run_task_with_delay(client_fixture: fixture) -> None:
    ...
    scheduled_task = [
        x for x in apscheduler.get_jobs() if len(x.args) > 0 and x.args[0] == str(t_id)
    ][0]
    assert (
...
def test_delete_all(client_fixture: fixture) -> None:
    ...
    assert len(apscheduler.get_jobs()) == 0
...
def test_kill(client_fixture: fixture) -> None:
    ...
    assert apscheduler.state == 0
    assert apscheduler.running == False
...
def test_details(client_fixture: fixture) -> None:
    ...
    job = apscheduler.get_job(f"{p_id}-{t.id}-cron")
    text = page.json[0]
...
def test_delete_orphans(client_fixture: fixture) -> None:
    ...
    assert len(apscheduler.get_jobs()) == 2
